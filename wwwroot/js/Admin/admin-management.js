document.addEventListener("DOMContentLoaded", () => {
  ...
  const filterTable = () => {
    ...
    tableRows.forEach((row) => {
      ...
      const rowPrice = priceCell
        ? parseFloat(priceCell.getAttribute("data-price"))
        : 0;
      ...
      row.style.display =
        matchesStatus && matchesSearch && matchesPrice ? "" : "none";
    });
  };
  ...
  window.deleteSelected = (type) => {
    ...
    if (
      !confirm(
        `Tem certeza que deseja excluir os ${selectedIds.length} itens selecionados?`,
      )
    ) {
      return;
    }
    ...
    const url =
      type === "flight" ? "/Admin/DeleteMassFlight" : "/Admin/DeleteMassHotel";
