document.addEventListener("DOMContentLoaded", () => {
  ...
  const updateUI = (hotel) => {
    ...
    if (DOM.reviewCount)
      DOM.reviewCount.textContent = `${hotel.reviews} reviews`;
    if (DOM.ratingText)
      DOM.ratingText.textContent = getRatingLabel(hotel.rating);
    if (DOM.overview)
      DOM.overview.textContent = hotel.overview || "Sem descrição disponível.";
    ...
    if (DOM.gallery) {
      ...
      (hotel.gallery || []).slice(0, 4).forEach((img, i) => {
        ...
        item.className =
          i === 3 ? "gallery-item view-all-container" : "gallery-item";
        item.innerHTML =
          i === 3
            ? `<img src="${img}"><button class="btn-view-all">Ver todas</button>`
            : `<img src="${img}">`;
