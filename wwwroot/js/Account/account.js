document.addEventListener("DOMContentLoaded", () => {
    ...
    uploadCoverInput.addEventListener("change", (e) => {
      ...
      reader.onload = (event) => {
        if (coverImageDisplay) {
          coverImageDisplay.style.backgroundImage = `url('${event.target.result}')`;
        }
      };
  ...
  const deleteBookingBtns = document.querySelectorAll(
    ".btn-delete-booking-ajax",
  );
  ...
  deleteBookingBtns.forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      ...
      const endpoint =
        type === "flight"
          ? "/Account/DeleteFlightBooking"
          : "/Account/DeleteStayBooking";
