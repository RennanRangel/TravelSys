document.addEventListener("DOMContentLoaded", () => {
  const DOM = {
    ...
    animatedElements: document.querySelectorAll(
      ".destination-card, .review-card, .feature-card",
    ),
  };
  ...
  DOM.tabBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (DOM.flightsForm)
        DOM.flightsForm.style.display = type === "flights" ? "flex" : "none";
      if (DOM.staysForm)
        DOM.staysForm.style.display = type === "stays" ? "flex" : "none";
    });
  });

  DOM.newsletterForm?.addEventListener("submit", (e) => {
    e.preventDefault();
    const email = DOM.newsletterForm.querySelector(
      'input[type="email"]',
    )?.value;
    if (email) {
  ...
  window.addEventListener("scroll", () => {
    ...
    DOM.header.style.background = isScrolled
      ? "rgba(255, 255, 255, 0.95)"
      : "transparent";
    DOM.header.style.backdropFilter = isScrolled ? "blur(10px)" : "none";
    DOM.header.style.boxShadow = isScrolled
  ...
  DOM.featureCards.forEach((card) => {
    card.querySelector(".btn-secondary")?.addEventListener("click", (e) => {
      ...
      window.location.href = isFlight
        ? "flights-listing.html"
        : "hotel-listing.html";
    });
  });
